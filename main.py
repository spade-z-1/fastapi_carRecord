...
@app.get("/api/car/carno", response_model=schemas.Car, name="获取指定车牌信息", description="获取车牌信息")
async def read_car(carno: str, db: Session = Depends(get_db)):
    ...
    :return:
    """
    db_car = crud.get_car_by_carno(db, carno=carno)
    if db_car is None:
...
async def create_record_for_car(record: schemas.RecordCreate, db: Session = Depends(get_db)):
    ...
    :return:
    """
    db_carno = crud.get_car_by_carno(db, carno=record.car_no)
    if db_carno is None:
        raise HTTPException(status_code=404, detail="car not found")
    return crud.create_car_record(db=db, record=record)


@app.get("/api/car/", name="获取所有车辆信息", description="获取所有车辆信息")
